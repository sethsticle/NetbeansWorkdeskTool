...
//    }
    ...
    // Gender combo box
    TableColumn genderColumn = tempUITable.getColumnModel().getColumn(2);
    genderColumn.setCellEditor(new DefaultCellEditor(new JComboBox<String>(new String[]{"He","She"})) {
        @Override
        public Component getTableCellEditorComponent(JTable table, Object value,
                                                     boolean isSelected, int row, int column) {
            JComboBox<String> combo = (JComboBox<String>) super.getTableCellEditorComponent(
                    table, value, isSelected, row, column);
            combo.setSelectedItem(value != null ? value : ""); // reset each time
            return combo;
        }
    });
}
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        if (!tempValidationUI.isSingleColumnPaste(name) || name.length() == 0) {
            JOptionPane.showMessageDialog(this, "Learner names must be a single column.");
            return;
        }
        if (!tempValidationUI.isSingleColumnPaste(surname) || surname.length() == 0) {
            JOptionPane.showMessageDialog(this, "Learner names must be a single column.");
            return;
        }
        if (!tempValidationUI.isSingleColumnPaste(mark1) || mark1.length() == 0) {
            JOptionPane.showMessageDialog(this, "Learner names must be a single column.");
            return;
        }
        if (!tempValidationUI.isSingleColumnPaste(mark2) || mark2.length() == 0) {
            JOptionPane.showMessageDialog(this, "Learner names must be a single column.");
            return;
        }
